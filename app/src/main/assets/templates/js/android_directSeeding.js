...
$(document).ready(function(){
			   $(".postImg img").on('click', function() {
                                        onThreadThumbsClicked();
                                   });;
...
function onRefresh(JSON){
    ...
    if(!textIsNull(JSON.Variables.postlist)){
        JSON.Variables.postlist.shift();
        var postlist = JSON.Variables.postlist;
    	var newpostlist = new Array();
    	for(var key in postlist) {
    	postlist[key].day =dateStr(parseInt(postlist[key].dbdateline));
    	if(typeof newpostlist[postlist[key].day] != 'undefined') {
    	    newpostlist[postlist[key].day][key] = postlist[key];
    	} else {
    		newpostlist[postlist[key].day] = {};
    		newpostlist[postlist[key].day][key] = postlist[key];
    	}
    	}
    	var data = "";
    	for(var i in newpostlist){
    	   if(mData != i){
                data +="<div class='ds-time'><span>"+i+"</span></div>";
                mData = i;
             }
//            mData = i;
//    	    data +="<div class='ds-time'><span>"+i+"</span></div>";
    	    for(var k  in newpostlist[i]){
                var datetime = newpostlist[i][k].dateline;
                var dbdateline = newpostlist[i][k].dbdateline;
                var message =newpostlist[i][k].message;
                var attach =newpostlist[i][k].attachments;
                var time= new Date(parseInt(dbdateline) * 1000);
                ...
                var append_img_box = '';
                 if(!textIsNull(attach)){
                    ...
                    append_img_box = append_img_box + "<a class='postImg'><img src='"+commendImgUrl+"'></a>";
                     }
                           append_img_box = append_img_box + "</div>";
                   }
                        data += "<div class='dsPostItem'><a><span class='dsTime'>"+time+"</span>"+message+"</a>";
                        data = data + append_img_box;
                        data = data + "</div><div class='margin10px'></div>";

    	    }

    	}
           $(".dsPostBox").append(data);
   }
...
function onLoadReply(JSON,isAppend){
    ...
    //                        var append_box = "<div class='dsPostItem'><a><span>"+datetime+"</span>"+message+"</a>";
                            data += "<div class='dsPostItem'><a><span class='dsTime'>"+time+"</span>"+message+"</a>";
                            data = data + append_img_box;
                            data = data + "</div><div class='margin10px'></div>";
...
/*@hidden*/function onThreadOptionsClick(){
 ...
 function dateStr(date){
           var time = new Date().getTime();
            ...
            var todayTime=(new Date(todayYear,todayMonth,todayDay)).getTime();//毫秒
            var datatime = parseInt(date*1000)  //输入时间

            if(datatime > todayTime) {
                return '今天';
            } else if(datatime < todayTime && (parseInt(todayTime - datatime)/1000) <= 3600*24) {
                return '昨天';
            } else if(datatime < todayTime && (parseInt(todayTime - datatime)/1000) <= 3600*24*2) {
                return '前天';
            } else {
